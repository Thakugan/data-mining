clean_dat <- function(dat) {
  ...
  dat <- mutate(dat,
                ...
                education = as.factor(cut(as.numeric(education),
                                           breaks = c(0, 3, 12, 16, Inf),
                                           labels = c(1, 2, 3, 4))),
                los = as.numeric(sub("-.*|\\+|<", "", los)),
                category = as.factor(category),
                supervisory_status = as.factor(supervisory_status))
  
  # TODO Create new binary features and do feature selection
  dat <- select(dat,
